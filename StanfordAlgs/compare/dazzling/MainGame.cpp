...
/* MainGame.cpp
	...
	$Id: MainGame.cpp,v 1.1.2.22.2.2.2.2.2.14 2015/06/18 14:07:55 mustafa.bialkov Exp $
...
{
		"NVRAM_DHP_2TWO_CHERRY",
		"NVRAM_DHP_3THREE_CHERRY",
		"NVRAM_DHP_4FOUR_CHERRY",
		"NVRAM_DHP_5FIVE_CHERRY",

		"NVRAM_DHP_3THREE_ORANGE",
		"NVRAM_DHP_4FOUR_ORANGE",
		"NVRAM_DHP_5FIVE_ORANGE",

		"NVRAM_DHP_3THREE_LEMON",
		"NVRAM_DHP_4FOUR_LEMON",
		"NVRAM_DHP_5FIVE_LEMON",

		"NVRAM_DHP_3THREE_PLUM",
		"NVRAM_DHP_4FOUR_PLUM",
		"NVRAM_DHP_5FIVE_PLUM",

		"NVRAM_DHP_3THREE_WATERMELON",
		"NVRAM_DHP_4FOUR_WATERMELON",
		"NVRAM_DHP_5FIVE_WATERMELON",

		"NVRAM_DHP_3THREE_GRAPES",
		"NVRAM_DHP_4FOUR_GRAPES",
		"NVRAM_DHP_5FIVE_GRAPES",

        "NVRAM_DHP_3THREE_SCATTER",
        "NVRAM_DHP_4FOUR_SCATTER",
        "NVRAM_DHP_5FIVE_SCATTER",

        "NVRAM_DHP_3THREE_SEVEN",
        "NVRAM_DHP_4FOUR_SEVEN",
        "NVRAM_DHP_5FIVE_SEVEN"
};
...
unsigned int MainGame::GetHitIDIndex( EFigure eFig, unsigned char ucFigCount )
{
    ...
    {
        eCDebug(LOG_CATEGORY_GAMES) << __FILE__ << ":" << __LINE__ << " GetHitIDIndex failed!";
        return 255;
    }
...
MainGame::MainGame()
        ...
        , m_ausHitIDIndx()
	 	...
	 	, m_bFGBgrSndStarted(false)
	 	, m_bFGBgrSndPaused(false)
...
bool MainGame::Init()
{
	...
	m_nFGTotalCount = 0;
	m_nFGMult = 1;
    m_crFGMotherWin = 0_dollars;
    m_crFGWin = 0_dollars;
    m_crPureFGWin = 0_dollars;
	m_bFGFastStoped = false;
	m_bNeedToRecordBonusWin = false;
    m_crExtraGameWin = 0_dollars;
	...
	if(!m_reelsArea.Init()) {
        eCDebug( LOG_CATEGORY_GAMES )
            <<"[ 5_DAZZLING_HOT ] MainGame::Init fail m_reelsArea " ;
		return false;
	}

	if(!m_FGSeals.Init()) {
        eCDebug( LOG_CATEGORY_GAMES )
            << "[5 Dazzling Hot] MainGame::Init m_FGSeals fail "; 
		return false;
	}

	if(!m_WinSplash.Init()) {
        eCDebug( LOG_CATEGORY_GAMES )
            << "[5 Dazzling Hot] MainGame::Init fail m_WinSplash ";
		return false;
	}

	if(!m_ExtraGame.Init() ) {
		eCDebug(LOG_CATEGORY_GAMES) << "[ 5_DAZZLING_HOT ] - m_ExtraGame::Init() FAIL  ";
		return false;
	}

	if(!m_BigWinDemo.Init() ) {
		eCDebug(LOG_CATEGORY_GAMES) << "[ 5_DAZZLING_HOT ] - m_BigWinDemo::Init() FAIL  ";
		return false;
	}
	...
	if( !GetParameter(nvram_parameter, (void*)&m_ucVariant, sizeof( m_ucVariant ) ) ){
		eCDebug( LOG_CATEGORY_GAMES ) << "Get NVRAM_DHP_GAME_VARIANT failed!\n";
	...
	m_sndBonusRetrig = gGameRM.GetSound( "BONUS_OUTRO" );
	...
	{
		eCDebug(LOG_CATEGORY_GAMES) << "[ 5_DAZZLING_HOT ] - JPA_KNOBS_BACKGROUND FAIL  ";
		return false;
	}
...
bool MainGame::StateChanged( bool bStackDirection, EGameStateDef gsdOldState, EGameStateDef gsdNewState )
{
	...
	// free games //////////////////////////////////////////////////////////////////
		...
		gpBasicSound->StartSound( m_sndBonusRetrig, false );
		m_reelsArea.SetScatterStarted( false );
	}
...
void MainGame::OnButton( const InputData &inputData )
{
	...
	if( m_bUserAutoMode ) {
		...
		if( inputData.uiButtons & BUTTON_STARTSTOP && inputData.uiPressed == BUTTON_STARTSTOP ){
            if( gMoneyCounting.GetLeftAmount() != 0_dollars ){
...
// 				DoStartButton(); //vk bug 19111
		...
		if( inputData.uiButtons == BUTTON_CASHOUT && inputData.uiPressed == BUTTON_CASHOUT ){ //vk bug 19111
          ...
          if ( gGPMgr.Credit_Get() > 0_dollars ){
			m_reelsArea.m_ActionMgr.StopAction();
		  }
...
//			SetLine( nLinesToSet );
			gBetLinesControl.DoButtonSelLine();
*/
		}
	}
...
//		}
	...
	if( inputData.uiButtons == BUTTON_STARTSTOP && inputData.uiPressed == BUTTON_STARTSTOP ){
        ...
        if( gMoneyCounting.GetLeftAmount() != 0_dollars ){
            ...
            else if( gMoneyCounting.GetLeftAmount() == 0_dollars && m_ucState == MG_STATE_WINSPLASH ){
	...
	if( inputData.uiButtons == BUTTON_AUTO && inputData.uiPressed == BUTTON_AUTO ){
		...
		if( !gGPMgr.GameSettings_Get().uchPlayerAutoMode &&
			...
			if( m_bUserAutoMode ){
                ...
                else if( gMoneyCounting.GetLeftAmount() != 0_dollars ) {
					gMoneyCounting.SetFastMode();
					SetDisabledButtons();
					return;
				}
...
{
	if( m_bTestAutoMode || m_bUserAutoModeON ){
		SetDisabledButtons();
		return;
	}
...
void MainGame::SetNewGameResults( unsigned char ucCombination )
{
    ...
    Currency crBetPerLine = gGPMgr.GameSettings_Get().cPointValue * Currency::Factor(gGPMgr.BetPerLine_Get());
	...
	else if( nID == MG_TIMER_REELING_DRAW )
	{
		...
		if( m_reelsArea.ReadyForScatter() )
		{
			...
			if( !m_bFG && !m_bTestAutoMode ){
				...
				m_ucState = MG_STATE_WINSPLASH;
				m_WinSplash.SetFGBonusWin( 15, 3 );
				if ( m_bFG )
				{
...
//				}
		...
		case MG_STATE_EXTRAGAME:
            if ( m_bWinGame && 0_dollars == gMoneyCounting.GetLeftAmount() )
			{
		...
		if( !m_WinSplash.IsShown() ){
			...
			if( m_WinSplash.GetMode() == WSPLASH_MODE_SCATTER_WIN ){
				...
				// change state (Betting or BettingPlus) /////////////////
                if(	IsDoubleUpOK() && (m_crFGWin > 0_dollars || m_crFGMotherWin > 0_dollars) ){
					m_ucState = MG_STATE_BETTING_PLUS;
					gGameStateMgr.RequestState( egsdMGBettingPlus );
				}
				else{
...
void MainGame::DoStartButton( bool bRecover )
{
  if(m_ucState == MG_STATE_REELING || gBetLinesControlAdapter.AllowPlay()){ //vk, || order matters
	...
	if( m_reelsArea.GetRAStatus() == RA_STATUS_STOPPED_READY && !m_bFG ){
		...
		int nLinesCount = gGPMgr.LinesCount_Get();
        Currency crBetPerLine = gGPMgr.GameSettings_Get().cPointValue * Currency::Factor(gGPMgr.BetPerLine_Get());
        Currency crTotalBet = crBetPerLine * Currency::Factor(nLinesCount);
		if( !bRecover ){
...
{
    auto gameType = GameVariantsComponent::GameType::MainGame;	// normal games
    if( m_bFG )
    {
        // in case of free games mode
...
void MainGame::GetRealResult( int nReel, unsigned char* ucResult, unsigned char ucMiddleIndex, bool bThereIsDoubt )
{
...
//		eCDebug(LOG_CATEGORY_GAMES) << "going fg with indexes";
	...
	GetSlidingWindow( gameType, ucVariant, nReel, ucMiddleIndex, ucResult);
}
...
//	size_t allowedRtpMainGame =  gameVariantsPtr->GetAllowedVariantsCount(); We need all game variants not only allowed
    ...
    for( int j = 0; j < REELS_COUNT; j++ ){
        ...
        for(  i = 0; i < VISIBLE_FIGURES_PER_REEL; i++ ){
	...
	for( i = 0; i < VISIBLE_FIGURES_PER_REEL; i++ ){
		for( int j = 0; j < REELS_COUNT; j++ ){
			ucCurrPos = AddVratkaData_uc( ucVratkaData, ucCurrPos, m_ucLogFigResults[i][j] );
		}
	}
	// denomination
...
void MainGame::Recover(SDump *pDump)
{
	...
	eCDebug(LOG_CATEGORY_GAMES) << "DUMP: pDump->ucMainGameState = " << (int)pDump->ucMainGameState  ;
	...
	if(gGameStateMgr.GetCurrentState() == egsdJPMProgressive)
	{
		if ( ((m_ucState != MG_STATE_BETTING_PLUS)&&(m_ucState != MG_STATE_BETTING) ) ||
           (m_AnalyzerResults.nWinLinesCount == 1 && m_AnalyzerResults.GetLineDataAt( 0 ).currWin == 0 ) || m_bFG )
		{
...
void MainGame::SetScatterLogData( TransactionId trID, unsigned char* pucMultipliers, unsigned char ucChoice )
{
	...
	const char* unCountsID = unNVRAMHitsIDs[GetHitIDIndex( ef7_scatter, ucScatterCount )];

	Currency crTotalBet = gGPMgr.GameSettings_Get().cPointValue;
    crTotalBet *= Currency::Factor(gGPMgr.TotalBet_Get());
    crTotalBet *= Currency::Factor(SCATTER_POSSIBLE_MULTIPLIERS[pucMultipliers[ucChoice]]);
    GameStatUnit_t gstat(crTotalBet, 0);
...
void MainGame::RecordRegularHits( TransactionId trID )
{
	...
	for( ; i < m_AnalyzerResults.GetWinLinesCount(); i++ ){
		...
		const char* unCountsID = unNVRAMHitsIDs[GetHitIDIndex( static_cast<EFigure> ( m_AnalyzerResults.GetLineDataAt( i ).eFigure ),
	...
	if( nScatterCount > 2 ){
		// counts //////////////////////////
		const char* unCountsID = unNVRAMHitsIDs[GetHitIDIndex( ef7_scatter, nScatterCount )];
...
void MainGame::FromBettingPlusToBetting( bool bTakeWin )
{
	...
	if( gGameStateMgr.GetCurrentState() == egsdMGBettingPlus ) {
        ...
        if( !bTakeWin && gGPMgr.Win_Get() != 0_dollars ){
...
bool MainGame::SwitchInJPState()
{
    ...
    if( gMoneyCounting.GetLeftAmount() == 0_dollars && m_nJPLine != (-1) ) {
...
int MainGame::OnHatStateChanged()
{
	...
	if( m_bFG ){
		...
		if ( gGPMgr.GetHatPrevState() == esdNonFatalError && !m_WinSplash.IsOutroShown() ){
			StopFGBgrSnd ();
			if ( MG_STATE_REELING == m_ucState )
				StartFGBgrSnd ();
		}
	...
	if ( gGPMgr.GetHatCurrState() == esdNonFatalError || gGPMgr.GetHatCurrState() == esdWarning )
	{
		...
		//not used
		//gPanelObject.SetDenomField( false );
...
void MainGame::SetFG( int nFGCount, int nFGMult, int nExtraWild )
{
	if( nFGCount > 0 ){
		...
		SetBettingButtons();
        m_crFGMotherWin = (m_crFGMotherWin != 0_dollars)? m_crFGMotherWin : gGPMgr.Win_Get();
...
void MainGame::StartFGBgrSnd()
{

}

void MainGame::StopFGBgrSnd()
{
}

void MainGame::PauseFGBgrSnd()
{

}

void MainGame::ResumeFGBgrSnd()
{
...
bool MainGame::IsDoubleUpOK(Currency crCheckedAmount)
{
    ...
    if ( crWin * Currency::Factor(2) > crGambleLimit ){
		return false;
	}

	// check for exceeded win limit
    Currency crGameWinLimit = gGPMgr.GameWinLimit_Get() / Currency::Factor(2);
    if ( crWin > crGameWinLimit && crGameWinLimit != 0_dollars )
		return false;
...
void MainGame::ResetDemoReelsArea() {
    ...
    for( unsigned int reelIndex = 0; reelIndex < REELS_COUNT; reelIndex++ )
    {
        ...
        for (auto nReelVariantFigure : vecReel)
        {
#ifdef EGT_DEBUG
    ...
    for( int line = 0; line < gGPMgr.LinesCount_Get(); line++ )
    {
        for( unsigned int reelIndex = 0; reelIndex < REELS_COUNT; reelIndex++ )
        {
            DemoReelsArea::InsertLines(line , gucWinLines[line][reelIndex]);
        }
    }
#endif
}
